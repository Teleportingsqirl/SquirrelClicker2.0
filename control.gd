...
func _process(_delta):
	...
	if not GameState.building_unlocked_mailbox.is_empty():
		...
		update_building_display() # Refresh display to show the new unlocked state
...
func _on_purchase_building_pressed():
	# Extra check to ensure we can't buy locked buildings
...
# --- All other functions in Control.gd remain the same ---
# ... (omitted for brevity)
func _start_death_sequence():
	GameState.death_mailbox = false
	var flash_rect = ColorRect.new()
	flash_rect.color = Color(1.0, 0, 0, 0); flash_rect.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	effects_layer.add_child(flash_rect)
	var flash_tween = create_tween().set_trans(Tween.TRANS_QUINT); var chain = flash_tween.chain()
	chain.tween_property(flash_rect, "color:a", 0.7, 0.1); chain.tween_property(flash_rect, "color:a", 0.0, 0.4)
	await flash_tween.finished; flash_rect.queue_free()
	var fade_rect = ColorRect.new()
	fade_rect.color = Color(0, 0, 0, 0); fade_rect.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	effects_layer.add_child(fade_rect)
	var death_tween = create_tween()
	death_tween.tween_property(fade_rect, "color:a", 1.0, 1.5).set_ease(Tween.EASE_IN)
	await death_tween.finished; GameState.save_game(); get_tree().quit()
...
func _update_single_buff(id: String, display_name: String, remaining: int, max_duration: int, color: Color, show_minutes: bool):
	if remaining > 0:
		...
		if not buff_ui_nodes.has(id):
			buff_bar = BuffBarScene.instantiate(); buff_container.add_child(buff_bar); buff_ui_nodes[id] = buff_bar
		...
		buff_ui_nodes[id].queue_free(); buff_ui_nodes.erase(id)
...
func _on_texture_button_pressed():
	...
	sfx_player.stream = SQUIRREL_CLICK_SFX; sfx_player.volume_db = -45.0
	add_child(sfx_player); sfx_player.play()
